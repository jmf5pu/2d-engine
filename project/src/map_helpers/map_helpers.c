...
void init_map(Map *map) {
  ...
  render_sprite_sheet_init(sprite_sheet_map_1_main_bg,
                           "assets/shooting_range_map.png", 1215, 560);
  ...
  Prop background_prop = (Prop){
      ...
      .layer_threshold = INT32_MIN, // this prop should ALWAYS render before
  ...
  background_prop.static_bodies[0] = physics_static_body_create(
      (vec2){607, 45}, (vec2){1230, 5}, COLLISION_LAYER_TERRAIN); // bottom
  ...
  background_prop.static_bodies[3] = physics_static_body_create(
      (vec2){1180, 280}, (vec2){5, 560}, COLLISION_LAYER_TERRAIN); // right
...
void update_pickup_status(Pickup *pickup) {
  ...
  // spawning to active
  else if (pickup->status == PICKUP_SPAWNING &&
           pickup->frames_on_status >=

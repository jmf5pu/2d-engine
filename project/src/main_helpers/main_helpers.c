...
bool should_destroy_entity(Entity *entity, Map *map)
{
    ...
    return (!entity->is_active || !entity->body->is_active) && !is_player && !is_pickup && !is_crosshair;
}
...
{
    return SPLIT_SCREEN ? (entity == player_one->crosshair->entity || entity == player_two->crosshair->entity) : (entity == player_one->crosshair->entity);
}
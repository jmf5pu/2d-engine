...
int main(int argc, char *argv[]) {
  ...
  while (!should_quit) {
    ...
    case GS_RUNNING:
      ...
      // init players and camera and spawn players if game is just
      // starting up
      if (!player_one) {
        if (SPLIT_SCREEN)
          set_render_dimensions(
              DEFAULT_RENDER_SCALE_FACTOR, false,
              false); // update the render width if split screen so
                      // that players will be centered correctly
        camera_init();
      ...
      // update relative position of players and their crosshairs
      // (transform of the position on the screen, using camera as a
      // vector)
      if (SPLIT_SCREEN) // use left cam if screen is split
      {
        player_one->relative_position[0] =
            player_one->entity->body->aabb.position[0] + left_cam.position[0];
        player_one->relative_position[1] =
            player_one->entity->body->aabb.position[1] + left_cam.position[1];
        ...
        player_one->crosshair->relative_position[1] =
            ...
            left_cam.position[1];
      } else // otherwise use main cam TODO: perhaps make left_cam and
             // main_cam the same object?
      {
        player_one->relative_position[0] =
            player_one->entity->body->aabb.position[0] + main_cam.position[0];
        player_one->relative_position[1] =
            player_one->entity->body->aabb.position[1] + main_cam.position[1];
        player_one->crosshair->relative_position[0] =
            player_one->crosshair->entity->body->aabb.position[0] +
            main_cam.position[0];
        player_one->crosshair->relative_position[1] =
      ...
      // need to pass the RELATIVE position of the players into the
      // physics engine to properly detect collisions
      ...
      if (SPLIT_SCREEN) {
        player_two->relative_position[0] =
            player_two->entity->body->aabb.position[0] + right_cam.position[0];
        player_two->relative_position[1] =
            player_two->entity->body->aabb.position[1] + right_cam.position[1];
      ...
      // update the actual positions (saved value plus the difference
      // generated by the physics engine)
      player_one->entity->body->aabb.position[0] =
          p1_pos_holder[0] + (player_one->entity->body->aabb.position[0] -
                              player_one->relative_position[0]);
      player_one->entity->body->aabb.position[1] =
      ...
      // need to run render loop twice if we are actively splitting the
      // screen
      int player_count = SPLIT_SCREEN ? 2 : 1;
      for (int i = 0; i < player_count; i++) {
        // FIRST:
        // For the player being rendered, update render dimensions based
        // on player status, update the corresponding projection matrix,
        // update the camera location, and clear the rendering buffer to
        // begin rendering THEN: handle the corresponding inputs and
        // update statuses/directions must handle player input BEFORE
        // rendering because entities can be created in
        ...
        // move all positions to relative position based on camera
        // except for the active player and crosshairs
        if (SPLIT_SCREEN && i == 0) {
          player_two->entity->body->aabb.position[0] =
              player_two->relative_position[0];
          player_two->entity->body->aabb.position[1] =
          ...
          update_all_positions(
              &map,
              (vec2){-1 * left_cam.position[0], -1 * left_cam.position[1]},
              true);
        } else if (SPLIT_SCREEN && i == 1) {
          player_one->entity->body->aabb.position[0] =
              player_one->relative_position[0];
          ...
          update_all_positions(
              &map,
              (vec2){-1 * right_cam.position[0], -1 * right_cam.position[1]},
              false);
        } else // not split
          update_all_positions(
              &map,
              (vec2){-1 * main_cam.position[0], -1 * main_cam.position[1]},
              true);

        // render animated entities, check if any are marked for
        // deletion (not active OR body is not active)
        ...
        for (int j = num_entities - 1; j >= 0; --j) {
          ...
          // destroy any entities that are inactive or have physics
          // bodies that are inactive and aren't associated with
          // players, crosshairs, or pickups
          bool is_crosshair = SPLIT_SCREEN
                                  ? (entity == player_one->crosshair->entity ||
                                     entity == player_two->crosshair->entity)
                                  : (entity == player_one->crosshair->entity);
          bool is_player = SPLIT_SCREEN ? (entity == player_one->entity ||
          ...
          if ((!entity->is_active || !entity->body->is_active) && !is_player &&
              !is_pickup && !is_crosshair) {
            entity_destroy(entity);
            continue;
          }

          // skip entities with no associated animations, check if
          // players and pickups are inactive
          ...
          animation_render(entity->animation, window,
                           entity->body->aabb.position, 0, game_color,
                           texture_slots);
        }
        ...
        for (int l = 0; l < map.num_props; l++) {
          ...
          /*
           * Determining the props z_index based on position relative
           * to player z_index should be below the players (0) if the
           * lowest point of the player is under the y threshold we
           * set OR they are above the prop entirely. Also the main
           * background, (index 0) is rendered beneath the player no
           * matter what
           */
          f32 player_y_min;
          if (i == 0) { // rendering left side
            player_y_min = player_one->entity->body->aabb.position[1] -
                           player_one->entity->body->aabb.half_size[1];
          } else { // rendering right side (if it exists)
            player_y_min = player_two->entity->body->aabb.position[1] -
                           player_two->entity->body->aabb.half_size[1];
          }
          bool is_below_player = player_y_min < (prop.sprite->position[1] -
          ...
          render_sprite_sheet_frame(
              prop.sprite->sprite_sheet, window, prop.sprite->row,
              prop.sprite->column, prop.sprite->position, z_index,
        ...
        // throw left side stuff into the rendering buffer, without
        // performing the screen swap yet
        if (SPLIT_SCREEN && i == 0) {
        ...
        if (SPLIT_SCREEN && i == 0) {
          ...
          update_all_positions(
              &map, (vec2){main_cam.position[0], main_cam.position[1]}, true);
      }
      render_end(window, texture_slots, false);

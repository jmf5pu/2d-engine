...
Entity *entity_create(vec2 position, vec2 size, vec2 velocity,
  ...
  Entity *entity =
      (Entity *)malloc(sizeof(Entity)); // Allocate memory for Entity
  if (entity == NULL) {
    // Handle memory allocation error
    ERROR_EXIT("Memory allocation failed\n");
  }
  entity->body = physics_body_create(position, size, velocity, collision_layer,
...
void entity_destroy(Entity *entity) {
  ...
  entity->body->is_active =
      false; // physics bodies for inactive entities should be inactive

...
void camera_init(void) {
  ...
  main_cam = (Camera){
      ...
      .buffer = {WINDOW_BUFFER, render_width - WINDOW_BUFFER, WINDOW_BUFFER,
                 render_height - WINDOW_BUFFER},
  };
  ...
  // half way to the right of the screen, buffers the center instead of the
  // left side
  right_cam = (Camera){
      .position = {0, 0}, // essentially bump everything to the right by half
                          // the screen width right off the bat
      .target_position = NULL,
      .buffer = {WINDOW_BUFFER, render_width - WINDOW_BUFFER, WINDOW_BUFFER,
                 render_height - WINDOW_BUFFER},
  };
...
void shift_camera(Player *player, vec2 shift) {
  // updates camera based on the passed in vector which represents the SHIFT,
  // not destination position
  vec2_sub(player->camera->position, player->camera->position, shift);
  vec2_add(player->entity->body->aabb.position,
           player->entity->body->aabb.position, shift);
  vec2_add(player->crosshair->entity->body->aabb.position,
           player->crosshair->entity->body->aabb.position, shift);
}
...
void shift_camera_smooth(Player *player, u32 delta) {
  ...
  f32 angle = fabs(
      (player->camera->target_position[0][0] > player->camera->position[0] &&
       player->camera->target_position[0][1] > player->camera->position[1]) ||
  ...
  vec2_sub(player->entity->body->aabb.position,
           player->entity->body->aabb.position, xy_magnitudes);
  vec2_sub(player->crosshair->entity->body->aabb.position,
           player->crosshair->entity->body->aabb.position, xy_magnitudes);
}

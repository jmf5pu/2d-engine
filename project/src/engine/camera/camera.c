...
void shift_camera(Player *player, vec2 shift)
{
    ...
    vec2_add(
        ...
        shift);
    vec2_add(
        player->crosshair->entity->body->aabb.position,
        player->crosshair->entity->body->aabb.position,
        shift);
}
...
{
    f32 dx =
        player->camera->position[0] - player->camera->target_position[0][0];
    f32 dy =
        player->camera->position[1] - player->camera->target_position[0][1];

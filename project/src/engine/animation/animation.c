...
void animation_render(Animation *animation, SDL_Window *window, vec2 position, vec4 color, u32 texture_slots[8])
{
    Animation_Definition *adef = animation->animation_definition;
    Animation_Frame *aframe = &adef->frames[animation->current_frame_index];
    render_sprite_sheet_frame(adef->sprite_sheet, window, aframe->row, aframe->column, position, animation->is_flipped, color, texture_slots);
}

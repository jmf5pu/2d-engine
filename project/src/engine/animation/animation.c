...
Animation_Definition *animation_definition_create(Sprite_Sheet *sprite_sheet,
                                                  f32 *durations, u8 *rows,
                                                  u8 *columns, u8 frame_count) {
...
void animation_update(f32 dt) {
  ...
  for (usize i = 0; i < animation_storage->len; ++i) {
    ...
    if (animation->is_active) {
      Animation_Definition *adef =
          animation->animation_definition; // get associated animation
                                           // definition
...
void animation_render(Animation *animation, SDL_Window *window, vec2 position,
                      i32 z_index, vec4 color, u32 texture_slots[32]) {
  ...
  render_sprite_sheet_frame(adef->sprite_sheet, window, aframe->row,
                            aframe->column, position, z_index,
                            animation->is_flipped, color, texture_slots);
}

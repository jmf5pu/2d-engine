...
static char *config_get_value(const char *config_buffer, const char *value) {
  ...
  if (!line) {
    ERROR_EXIT("Could not find config value: %s. Try deleting config.ini "
               "and restarting.\n",
               value);
  }
...
static void load_controls(const char *config_buffer) {
  ...
  config_key_bind(INPUT_KEY_L_RIGHT,
                  config_get_value(config_buffer, "l_right"));
  ...
  config_key_bind(INPUT_KEY_L_SHOOT,
                  config_get_value(config_buffer, "l_shoot"));
  config_key_bind(INPUT_KEY_L_CROUCH,
                  config_get_value(config_buffer, "l_crouch"));
  config_key_bind(INPUT_KEY_L_RELOAD,
                  config_get_value(config_buffer, "l_reload"));

  // right player keybinds
  config_key_bind(INPUT_KEY_R_LEFT, config_get_value(config_buffer, "r_left"));
  config_key_bind(INPUT_KEY_R_RIGHT,
                  config_get_value(config_buffer, "r_right"));
  ...
  config_key_bind(INPUT_KEY_R_SHOOT,
                  config_get_value(config_buffer, "r_shoot"));
  config_key_bind(INPUT_KEY_R_CROUCH,
                  config_get_value(config_buffer, "r_crouch"));
  config_key_bind(INPUT_KEY_R_RELOAD,
                  config_get_value(config_buffer, "r_reload"));
  ...
  config_key_bind(INPUT_KEY_RIGHT,
                  config_get_value(config_buffer, "menu_right"));

...
void render_init_shaders(u32 *shader_default, u32 *shader_batch,
  ...
  *shader_batch = render_shader_create("./shaders/batch_quad.vert",
                                       "./shaders/batch_quad.frag");
  update_projection_matrix(shader_default, shader_batch, render_width,
                           render_height);
}

void update_projection_matrix(u32 *shader_default, u32 *shader_batch,
                              f32 render_width, f32 render_height) {
  mat4x4 projection;
...
void render_init_batch_quads(u32 *vao, u32 *vbo, u32 *ebo) {
  ...
  glGenBuffers(1, vbo);
  glBindBuffer(GL_ARRAY_BUFFER, *vbo);
  glBufferData(GL_ARRAY_BUFFER, MAX_BATCH_VERTICES * sizeof(Batch_Vertex), NULL,
               GL_DYNAMIC_DRAW);
  ...
  glVertexAttribPointer(2, 4, GL_FLOAT, GL_FALSE, sizeof(Batch_Vertex),
                        (void *)offsetof(Batch_Vertex, color));
  glEnableVertexAttribArray(3);
  glVertexAttribPointer(3, 1, GL_FLOAT, GL_FALSE, sizeof(Batch_Vertex),
                        (void *)offsetof(Batch_Vertex, texture_slot));

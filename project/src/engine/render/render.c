...
void set_render_dimensions(f32 scale_factor, bool rendering_hud,
                           bool update_shaders) {
  ...
  if (update_shaders) {
    update_projection_matrix(&shader_default, &shader_batch, render_width,
                             render_height);
  }
}
...
static void render_batch(Batch_Vertex *vertices, usize count,
                         u32 texture_ids[32]) {
  glBindBuffer(GL_ARRAY_BUFFER, vbo_batch);
  // Iterate through the list_batch array list and access each Batch_Vertex
  // instance

  qsort(list_batch->items, list_batch->len, sizeof(Batch_Vertex *),
        compare_vertices);
...
void render_end(SDL_Window *window, u32 batch_texture_ids[32],
                bool swap_window) {
...
static void append_quad(vec2 position, vec2 size, vec4 texture_coordinates,
                        i32 z_index, vec4 color, f32 texture_slot) {
  vec4 uvs = {0, 0, 1, 1};
  ...
  *vertices[0] =
      (Batch_Vertex){.position = {position[0], position[1]},
                     .uvs = {uvs[0], uvs[1]},
  ...
  *vertices[1] =
      (Batch_Vertex){.position = {position[0] + size[0], position[1]},
                     .uvs = {uvs[2], uvs[1]},
                     .color = {color[0], color[1], color[2], color[3]},
                     .texture_slot = texture_slot,
                     .z_index = z_index,
  ...
  *vertices[2] =
      (Batch_Vertex){.position = {position[0] + size[0], position[1] + size[1]},
                     .uvs = {uvs[2], uvs[3]},
                     .color = {color[0], color[1], color[2], color[3]},
                     .texture_slot = texture_slot,
                     .z_index = z_index,
  ...
  *vertices[3] =
      (Batch_Vertex){.position = {position[0], position[1] + size[1]},
                     .uvs = {uvs[0], uvs[3]},
                     .color = {color[0], color[1], color[2], color[3]},
                     .texture_slot = texture_slot,
                     .z_index = z_index,
...
void render_sprite_sheet_init(Sprite_Sheet *sprite_sheet, const char *path,
...
void render_sprite_sheet_frame(Sprite_Sheet *sprite_sheet, SDL_Window *window,
                               f32 row, f32 column, vec2 position, i32 z_index,
                               bool is_flipped, vec4 color,
  ...
  if (texture_slot == -1) {
    render_end(window, texture_slots,
               false); // render all the batched frames before resetting for
                       // another write
